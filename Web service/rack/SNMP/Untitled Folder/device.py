...
# function readInputData read raw input data from inputIp table in database
def readInputData(db):
	sql = "SELECT * FROM inputIp;"
	try:
		cursor = db.cursor()
		cursor.execute(sql)
...
# function ipNeighborhood find devices that is connected to it 
def ipNeighborhood(db, ip, communityStrings):
	ipNeighbor = []
	session = Session(hostname =ip, community =communityStrings, version =2)
	result = session.walk('IP-MIB::ipAdEntAddr')
	for x in result:
		ipNeighbor.append(x.value.encode('ascii', 'ignore'))

	return ipNeighbor


# function returnIpList return ip list that is unique with format: [(ip1, communityString1), ..] 
def returnIpList(db):
	inputData = readInputData(db)

	ipDeviceInfo = []
	neighbor = []
	for ip, communityStrings in inputData:
		if ip in neighbor or ip in [x[0] for x in ipDeviceInfo]:
			continue
		ipNeighbor = ipNeighborhood(db, ip, communityStrings)
		ipDeviceInfo.append(ip)
		ipNeighbor.remove(ip)
		neighbor += ipNeighbor

	ipList = []
	for ip, communityStrings in inputData:
		if ip in ipDeviceInfo:
			ipList.append((ip, communityStrings))
